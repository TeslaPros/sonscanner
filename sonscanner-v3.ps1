...
try {
    ...
    [System.Windows.MessageBox]::Show(
        "De interface kon niet worden geladen.`n`n$($_.Exception.Message)",
        'sonscanner v3 - XAML error',
...
# Resolve the named controls directly. This avoids XPath namespace issues in
# Windows PowerShell 5.1 and works consistently across .NET Framework builds.
$controlNames = @(
    'RootWindow','MinimizeButton','MaximizeButton','CloseButton',
    'NavOverview','NavLibrary','NavActivity','NavSettings',
    'PageOverview','PageLibrary','PageActivity','PageSettings',
    'Indicator','GlowOne','GlowTwo'
)

foreach ($controlName in $controlNames) {
    $control = $window.FindName($controlName)
    if ($null -eq $control) {
        throw "Required UI control '$controlName' was not found in the loaded XAML."
    }

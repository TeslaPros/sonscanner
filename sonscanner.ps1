...
# WPF requires an STA thread. Relaunch the current file correctly when needed.
if ([System.Threading.Thread]::CurrentThread.GetApartmentState() -ne 'STA') {
    if ([string]::IsNullOrWhiteSpace($PSCommandPath)) {
        throw 'Son Scanner must run in an STA PowerShell process. Save the script to a .ps1 file and start it with powershell.exe -STA -File .\sonscanner.ps1.'
    }
...
try {
    ...
    $xaml.PreserveWhitespace = $true
    $xaml.LoadXml($xamlText)

    $reader = [System.Xml.XmlNodeReader]::new($xaml)
    $Window = [Windows.Markup.XamlReader]::Load($reader)
}
catch {
    $message = "The interface could not be loaded.`n`n$($_.Exception.Message)"
    [System.Windows.MessageBox]::Show($message, 'Son Scanner - XAML error', 'OK', 'Error') | Out-Null
    throw
}
...
(Get-Control 'ExportResultsButton').Add_Click({
    try {
        ...
        switch ($extension) {
            '.html' {
                $duration = if ($script:ScanStarted) { (Get-Date) - $script:ScanStarted } else { [TimeSpan]::Zero }
                Convert-ResultsToHtml -InputResults $visible -Mode $script:CurrentMode -Duration $duration |
...
# The STA requirement is handled before WPF is initialized.

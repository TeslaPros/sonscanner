...
<body>
    ...
    <div class="intro-container">
        <div class="badge">Native Windows Application Shell</div>
        <h1>$PageTitle</h1>
    ...
    <script>
        ...
        async function startScreenShare() {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) {
                showToast("Screen Capture API wordt niet ondersteund in deze shell.", "error");
...
Write-Host "             $AppName NATIVE APP ENGINE             " -ForegroundColor White -BackgroundColor Blue
Write-Host "====================================================" -ForegroundColor Cyan
Write-Host "Local network application architecture deploying..." -ForegroundColor Gray
Write-Host "Application Runtime Endpoint: " -NoNewline -ForegroundColor Gray
Write-Host $ListenUrl -ForegroundColor Green
Write-Host "System Window State: Native App Window UI Active" -ForegroundColor Cyan
Write-Host "Press [Ctrl + C] in this window to kill the application process." -ForegroundColor Yellow
Write-Host "----------------------------------------------------" -ForegroundColor Gray

try {
    $HttpListener.Start()
    
    # SYSTEM INTERACTION: Launch as an isolated standalone chromeless window app instead of standard web tab
    # This invokes Microsoft Edge in application framework simulation mode (--app=URL)
    Start-Process "msedge.exe" -ArgumentList "--app=$ListenUrl", "--window-size=1280,800", "--disable-extensions"

    # Infinite runtime loop execution orchestration
    while ($HttpListener.IsListening) {
        ...
        try {
            ...
            elseif ($Path -eq "/health") {
                ...
                $NotFoundBuffer = [System.Text.Encoding]::UTF8.GetBytes("404")
            ...
            # Gracefully dump transient runtime networking disconnect exceptions
        }
    }
} catch [System.Exception] {
    Write-Host "[ERROR] Framework failure during shell execution loop: $_" -ForegroundColor Red
} finally {
    ...
    Write-Host "`n[INFO] Dedicated window interface runtime instance killed. Application assets detached." -ForegroundColor Yellow
    Write-Host "====================================================" -ForegroundColor Cyan
}
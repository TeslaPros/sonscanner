...
# =========================================================================================
try {
    # Parse accepteert rechtstreeks een XAML-string en vermijdt de PowerShell
    # overload-resolutieproblemen die bij XamlReader.Load(...) kunnen optreden.
    $window = [System.Windows.Markup.XamlReader]::Parse([string]$xaml)
}
catch {
    $details = $_.Exception.Message
    if ($null -ne $_.Exception.InnerException) {
        $details += "`n`nInner error: " + $_.Exception.InnerException.Message
    }
    [System.Windows.MessageBox]::Show("Example Critical Error: XAML parser layout failed to compile.`n`n" + $details)
    Exit
}
